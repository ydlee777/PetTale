...
class OreamySessionIssuerTests {
    ...
    @Test void thirtyDayLifetimeProducesThirtyDayExpiration() {
        var issuer = new OreamySessionIssuer(encoder, Clock.fixed(NOW, ZoneOffset.UTC), "oreamy-backend", Duration.ofDays(30));
    ...
    @Test void configuredCustomLifetimeOverridesDefault() {
        var issuer = new OreamySessionIssuer(encoder, Clock.fixed(NOW, ZoneOffset.UTC), "oreamy-backend", Duration.ofHours(2));
