...
import com.oreamy.backend.usage.AiUsageStatus;
...
@SpringBootTest(properties = {
        ...
        "oreamy.ai.free-monthly-request-limit=1",
        "oreamy.ai.premium-monthly-request-limit=1"
})
class AiExtractionIntegrationTests {
    ...
    @Test void authenticatedRequestReservesBeforeProviderAndRecordsSuccessMetadata() throws Exception {
        ...
        var unchangedTrial = users.findById(user.getId()).orElseThrow();
        assertThat(unchangedTrial.getTrialStartedAt()).isEqualTo(NOW);
        assertThat(unchangedTrial.getTrialExpiresAt()).isEqualTo(NOW.plus(java.time.Duration.ofDays(30)));
    }
    ...
    @Test void onlyJwtIdentityOwnsUsageAndPrivateContextIsNotPersisted() throws Exception {
        ...
        assertThat(com.oreamy.backend.usage.AiUsage.class.getDeclaredFields())
    ...
    private ServiceUser user(String subject) {
        ...
        user.activateTrialIfEligible(NOW, java.time.Duration.ofDays(30));
