...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, properties = {
        "spring.datasource.url=jdbc:h2:mem:pettale;MODE=PostgreSQL;DB_CLOSE_DELAY=-1",
        "spring.datasource.username=sa", "spring.datasource.password=",
        "spring.jpa.hibernate.ddl-auto=validate",
        "pettale.session.signing-key=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=",
        "pettale.apple.audience=com.pettale.app"
})
class AppleAuthenticationIntegrationTests {
    ...
    @Test void healthIsPublicAndSessionEndpointRequiresValidPettaleToken() throws Exception {
    ...
    @Test void issuedPettaleTokenAuthenticatesInternalUserIdentity() throws Exception {
    ...
    @Test void expiredPettaleSessionIsRejected() throws Exception {
        var now = Instant.now();
        var claims = JwtClaimsSet.builder()
                .issuer("pettale-backend")
