        ...
        NavigationStack {
            ...
            .onChange(of: authentication.state) { _, state in
                if case .signedIn(let session) = state, controller.phase == .authenticationRequired {
                    ...
                    Task { await performExtraction(session: session) }
                }
            }
            .task {
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            HStack {
                ...
                Button("Continue") {
                    ...
                    .buttonStyle(.borderedProminent)
                    .accessibilityHint("Extract event drafts using the Oreamy service")
            }
    ...
    private var authenticationRequiredContent: some View {
        VStack(spacing: 18) {
            ...
            Text("Sign in to Extract Events")
                .font(.title2.bold())
            Text("Your transcript remains here while you sign in to the Oreamy service.")
                .multilineTextAlignment(.center)
                .foregroundStyle(.secondary)
