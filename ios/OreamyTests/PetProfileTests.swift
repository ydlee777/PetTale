...
final class PetProfileTests: XCTestCase {
    ...
    func testNoPetsRoutesToWelcomeAfterIntro() {
    ...
    func testDeletingPetCascadesRecordsAndEventsAndPreservesOtherPet() throws {
        let container = try makeContainer()
        let oreo = try Pet(name: "Oreo", species: .cat)
        let creamy = try Pet(name: "Creamy", species: .cat, profilePhotoData: Data([1, 2, 3]))
        let oreoRecord = try PetRecord(pet: oreo, originalTranscript: "Oreo played.")
        let creamyRecord = try PetRecord(pet: creamy, originalTranscript: "Creamy ate.")
        let oreoEvent = try PetEvent(record: oreoRecord, category: .activity, eventType: "PLAY")
        let creamyEvent = try PetEvent(record: creamyRecord, category: .food, eventType: "MEAL")
        container.mainContext.insert(oreo)
        container.mainContext.insert(creamy)
        container.mainContext.insert(oreoRecord)
        container.mainContext.insert(creamyRecord)
        container.mainContext.insert(oreoEvent)
        container.mainContext.insert(creamyEvent)
        try container.mainContext.save()

        try PetDeletionService.delete(creamy, from: container.mainContext)

        XCTAssertEqual(try container.mainContext.fetch(FetchDescriptor<Pet>()).map(\.name), ["Oreo"])
        XCTAssertEqual(try container.mainContext.fetch(FetchDescriptor<PetRecord>()).map(\.id), [oreoRecord.id])
        XCTAssertEqual(try container.mainContext.fetch(FetchDescriptor<PetEvent>()).map(\.id), [oreoEvent.id])
    }

    func testPetDeletionPersistsAfterStoreReopen() throws {
        let directory = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString, directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let storeURL = directory.appending(path: "PetDeletion.store")

        do {
            let container = try PettalePersistence.makeModelContainer(cloudKitEnabled: false, storeURL: storeURL)
            let pet = try Pet(name: "Creamy", species: .cat)
            let record = try PetRecord(pet: pet, originalTranscript: "Creamy played.")
            let event = try PetEvent(record: record, category: .activity, eventType: "PLAY")
            container.mainContext.insert(pet)
            container.mainContext.insert(record)
            container.mainContext.insert(event)
            try container.mainContext.save()
            try PetDeletionService.delete(pet, from: container.mainContext)
        }

        let reopened = try PettalePersistence.makeModelContainer(cloudKitEnabled: false, storeURL: storeURL)
        XCTAssertTrue(try reopened.mainContext.fetch(FetchDescriptor<Pet>()).isEmpty)
        XCTAssertTrue(try reopened.mainContext.fetch(FetchDescriptor<PetRecord>()).isEmpty)
        XCTAssertTrue(try reopened.mainContext.fetch(FetchDescriptor<PetEvent>()).isEmpty)
    }
}
