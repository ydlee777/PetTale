...
final class AuthenticationControllerTests: XCTestCase {
    ...
    func testNewControllerLifecycleAutomaticallyRestoresSavedSession() async throws {
        let session = makeSession()
        let store = MemorySessionStore()
        let first = AuthenticationController(service: FakeAuthenticationService(result: .success(session)), store: store)
        _ = try first.prepareNonce()
        await first.completeAppleAuthorization(identityTokenData: Data("apple.jwt".utf8))

        let relaunched = AuthenticationController(service: FakeAuthenticationService(), store: store)
    ...
    func testBackendUnauthorizedInvalidatesStoredSession() {
        let store = MemorySessionStore(session: makeSession())
        let controller = AuthenticationController(service: FakeAuthenticationService(), store: store)
    ...
    func testKeychainStoreRoundTripAndClear() throws {
        let store = KeychainSessionStore(service: "com.oreamy.tests.\(UUID().uuidString)")
    ...
    private func makeSession() -> OreamySession {
        OreamySession(userID: UUID(), accessToken: "oreamy.jwt", expiresAt: Date().addingTimeInterval(900))
    }
}

private final class MemorySessionStore: SessionStore {
    var session: OreamySession?
    var didClear = false
    init(session: OreamySession? = nil) { self.session = session }
    func load() throws -> OreamySession? { session }
    func save(_ session: OreamySession) throws { self.session = session }
    func clear() throws { session = nil; didClear = true }
}

private final class FakeAuthenticationService: AuthenticationService {
    var result: Result<OreamySession, Error>
    var receivedToken: String?
    var receivedNonce: String?
    init(result: Result<OreamySession, Error> = .failure(AuthenticationError.invalidBackendResponse)) { self.result = result }
