...
struct PeriodStatisticsView: View {
    ...
    init(pet: Pet, recordAction: @escaping () -> Void) {
        ...
        let requested = arguments.firstIndex(of: "-pettaleStatisticsPeriod").flatMap { index in
    ...
    private var eventSummary: some View {
        let metrics: [(Int, LocalizedStringKey, String)] = [
            (statistics.foodCount, "Food Records", "fork.knife"),
            (statistics.activityCount, "Activity Records", "figure.run"),
            (statistics.healthCount, "Health Records", "heart.text.clipboard"),
            (statistics.vetCount, "Vet Records", "cross.case")
        ]
